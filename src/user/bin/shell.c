...
static void cmd_help(const char *topic) {
    ...
    if (!topic[0]) {
        puts("commands: ls cd pwd stat about health interfaces fsstat fdstat cat mkdir rmdir touch write rm mv nano basm gui apps guidemo notes forms calc ping wget tcptwotest dhcp netstat syncstat elfbadtest pipetest pipeedgetest pipeblocktest futextest futextimeouttest futexcanceltest futexblocktest threads exec wait kill ps echo sleep reboot help");
        ...
        puts("quick start: about; health; interfaces; gui; apps; apps info forms; forms; calc");
        return;
    }
    ...
    if (strcmp(topic, "proc") == 0) {
        ...
        puts("try: about; health; interfaces; ls /proc; cat /proc/about; cat /proc/health; cat /proc/interfaces");
...
static void execute(char *line) {
    ...
    else if (starts_with(line, "about")) cmd_about();
