...
进入后先显示 APP MANAGER，由用户选择 Paint、Shell 或 `/fs/apps` 里的外部 GUI 程序。Paint 用鼠标绘图；Shell 支持 `help`、`ls`、`cat`、`echo`、`apps`、`run <path>` 等内置命令。把 ELF32 GUI 程序放到 `/fs/apps/<name>` 后，可在管理器内点击或在 GUI Shell 里用 `run /fs/apps/<name>` 启动；外部 GUI 程序退出后会回到管理器。应用内按 `Esc` 或 `Ctrl+C` 返回管理器，管理器内再按一次返回文本 shell。
