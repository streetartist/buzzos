...
- 读懂并修改 multiboot2 入口、中断入口、syscall 入口和上下文切换代码。
...
- [12. 内核汇编函数](#12-内核汇编函数)
- [13. Multiboot2 入口汇编](#13-multiboot2-入口汇编)
- [14. 早期内核入口和段状态](#14-早期内核入口和段状态)
...
| 文件 | 构建格式 | 作用 |
| --- | --- | --- |
| [src/kernel/arch/i386/mb2_entry.asm](../src/kernel/arch/i386/mb2_entry.asm) | `nasm -f elf32` | multiboot2 header 和内核早期入口 |
...
BuzzOS 当前主线由 Limine 从 FAT16 boot 分区加载 `kernel.elf`。内核汇编、用户态汇编和中断入口都以 ELF32 object 形式参与链接。
...
### 2.4 `section`、`global`、`extern`

BuzzOS 的汇编文件作为 ELF32 object 链接进内核或用户程序，因此不要在这些文件里写 `org`。地址由链接脚本和 ELF program header 决定，汇编源码只需要放进明确的 section：
...
Limine 按 ELF program header 把内核加载到链接脚本指定的地址。当前不再由自写早期加载器固定读取扇区并复制内核。
...
当前 BuzzOS 的启动主线由 Limine 加载 `kernel.elf`。内核侧的第一个汇编入口是 [src/kernel/arch/i386/mb2_entry.asm](../src/kernel/arch/i386/mb2_entry.asm)。

### 13.1 multiboot2 header

内核 ELF 里必须有 multiboot2 header，让 Limine 能识别它支持的启动协议。这个 header 放在内核镜像早期区域，由链接脚本保证位置。
...
这里的校验和规则是：magic、architecture、header length、checksum 四项相加为 0。

### 13.2 入口寄存器

Limine 按 multiboot2 协议进入内核时，关键寄存器是：

```text
EAX = multiboot2 bootloader magic
EBX = multiboot2 information structure pointer
```

早期入口必须把它们传给 C 内核。不要在建立 C 调用参数前随便覆盖 `EAX/EBX`。
...
入口里把 `ESP` 指到 `boot_stack_top`，再按 cdecl 压参数调用 C 函数。
...
实际文件名和 C 函数名以仓库源码为准。关键点是：汇编入口只做必须的启动桥接，复杂解析放到 C 里。

### 13.5 framebuffer 和 memory map

Limine 设置 framebuffer 后，multiboot2 information structure 会包含 framebuffer tag。C 内核解析 tag 后调用 framebuffer 驱动保存：

```text
addr, width, height, pitch, bpp
```

同理，物理内存布局来自 multiboot2 memory map tag，而不是内核自己在 16 位实模式里调用 BIOS E820。

---

## 14. 早期内核入口和段状态

当前 BuzzOS 不在内核早期入口里完成 BIOS 磁盘读取或保护模式切换。Limine 进入内核时，CPU 已经处于 32 位可执行环境，内核入口只需要建立自己的早期栈并把 multiboot2 参数交给 C。

这并不意味着 GDT/TSS 不重要。内核仍然会在 C 初始化阶段安装自己的 GDT 和 TSS：

```text
mb2_entry -> kernel_main -> gdt_install -> idt_install -> paging_init
```

GDT/TSS 的关键作用：

- 提供内核代码段和数据段描述符。
- 提供用户态 ring 3 代码段和数据段描述符。
- 通过 TSS 提供从用户态进入内核态时要切换到的 kernel stack。

### 14.1 汇编入口应该保持很薄

早期入口不应该解析复杂数据结构，也不应该做大量设备初始化。推荐分工是：

```text
汇编入口：设置栈、保存启动参数、调用 C
C 内核：解析 multiboot2 tag、初始化驱动、建立分页和用户态
```

这样做的好处是调试更简单：如果启动卡在汇编入口，排查栈和传参；如果进入 C 后出错，串口日志和 C 代码结构会更容易定位。
...
内核串口日志会输出到当前终端。汇编导致崩溃时，串口日志通常比 framebuffer 画面更可靠。
...
当前主线由 Limine 加载 `kernel.elf`，早期汇编入口是
[src/kernel/arch/i386/mb2_entry.asm](../src/kernel/arch/i386/mb2_entry.asm)。
这里出错时，内核串口还没完全初始化，常见排查顺序是：

1. 确认 multiboot2 header 仍在镜像前部，Limine 能识别。
2. 确认入口栈没有越界或被覆盖。
3. 确认传给 C 入口的 `EAX`/`EBX` 没被破坏。
4. 用 QEMU 串口和 Limine panic 信息先定位到“是否进入内核”。
...
1. 在 `mb2_entry.asm` 里临时改变传给 C 入口前的栈布局，观察异常信息。
2. 给 multiboot2 header 添加或调整请求 tag，理解 Limine 如何选择 framebuffer。
3. 在 C 入口最早期打印 multiboot2 magic 和 info 指针。
4. 解析一个新的 multiboot2 tag，并把结果输出到串口。
5. 不建议一开始就重写 bootloader。先读懂 Limine 配置、multiboot2 header 和入口寄存器约定，再做小改。
...
| --- | --- |
| 内核链接起始地址 | `0x100000` |
| multiboot2 入口文件 | `src/kernel/arch/i386/mb2_entry.asm` |
| 默认 framebuffer 请求 | `1280x800x32` |
| 保护模式早期栈 | `0x700000` |
| 用户程序链接地址 | `0x200000` |
...
5. 最后再改 `mb2_entry.asm` 或 Limine 配置，因为早期入口状态少、失败时日志也少。
