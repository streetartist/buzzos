...
目标：让种子用户 GUI 示例有同一套轻量视觉语言，避免每个 app 重复维护自己的顶栏、面板、按钮、文本框、指针和状态色画法。

涉及文件：
- `src/user/libc/gui_style.h`
- `src/user/bin/guidemo.c`
- `src/user/bin/notes.c`
- `src/user/bin/forms.c`
- `src/user/bin/calc.c`
- `Makefile`
- `tools/check_project.py`
- `tools/project_report.py`
...
已完成：
- 新增 `src/user/libc/gui_style.h`，提供 shared `ui_topbar`、`ui_panel`、`ui_button`、`ui_field`、`ui_textbox`、`ui_list_row`、`ui_scrollbar`、滚轮状态 helper 和 `ui_pointer`。
- `guidemo`、`notes`、`forms`、`calc` 改用同一套 helper 绘制顶栏、面板、按钮、文本框、hover/focus 边框、状态色和指针。
- `Makefile` 增加 `USER_HEADERS` 依赖，修改 `gui_style.h` 会触发用户 GUI app 重编。
- `make report` 增加 `## GUI Style` 表，展示共享样式头和 app 接入情况。
- `make check-project` 静态检查四个种子 app 使用共享样式，并拒绝继续保留重复的本地 `border`/`button`/`draw_pointer` helper。

验收标准：
- 四个种子用户 GUI app 都包含 `gui_style.h`。
- 四个种子用户 GUI app 都使用 shared 顶栏、面板、按钮和指针 helper。
- `make check-project` 通过。
