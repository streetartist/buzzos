...
- `make report` 输出 Project Identity、Health Interfaces、Runtime Interfaces 和 Runtime Limits。
- `make check-project` 静态检查 procfs、文本 shell、GUI shell、报告和 smoke 覆盖。
- `make smoke` 在 QEMU 中实际读取 `/proc/about`、`/proc/health`、`/proc/interfaces`、`/proc/limits` 并执行 `about`、`health`、`interfaces`、`limits`。
...
目标：在用户构建或启动 QEMU 前，用轻量宿主脚本提前发现工具链、QEMU 路径和工作区文件缺失问题。
...
- `make report` 输出 Host Doctor 表，并用 soft 模式避免报告因缺少某个本地工具直接失败。
- `make check-project` 静态检查 doctor、报告和 README 覆盖。

验收标准：
- `make doctor QEMU="C:\Program Files\qemu\qemu-system-i386.exe"` 能输出宿主环境检查结果。
- `make report` 包含 `## Host Doctor`。
